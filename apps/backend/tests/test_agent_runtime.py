...
from agents.errors import AgentConflictError, AgentContractError, DuplicateJobError
...
def test_agent_api_exposes_lifecycle_and_duplicate_protection(client):
    ...
    client.app.state.agent_runtime.providers.register(provider)
    payload = job().model_dump(mode="json")

    created = client.post("/api/v1/agents", json=payload)
    duplicate = client.post("/api/v1/agents?run_now=false", json=payload)
    audit = client.get(f"/api/v1/agents/{payload['job_id']}/audit")
